...
for iter in range(120): 
    ...
    top_k = pop.tell_pop(reward, k=9) # assign reward to topologies
    top_k = pop.train_n_pick(top_k, k=3) # pick best topologies under backprop
    pop.update_pop(top_k, power = (iter+1)//2, perturb_update=True) # perturb trained individuals and put back into population 
